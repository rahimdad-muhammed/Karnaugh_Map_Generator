...
class MapGenerator:
	def __init__(self, settings) -> None:
		...
		#self.triangle_row_colour = (0,0,0)
		self.triangle_row_pos = [self.cell_start.topleft, self.cell_start.bottomleft, self.cell_start.bottomright]

		#self.triangle_column_colour = (0,0,0)
		self.triangle_column_pos = [self.cell_start.topleft, self.cell_start.topright, self.cell_start.bottomright]


		
		match settings.format:
			case "row" if self.settings.generate_map:
				...
				for input_index, input_type in enumerate(self.settings.input_value.keys()):

					if input_index <= self.row_in - 1:
						settings.row_inputs = settings.row_inputs + input_type

					else:
						settings.column_inputs = settings.column_inputs + input_type

				self.settings.row_inputs_surf = settings.font.render(settings.row_inputs,True, (0,0,255))

				self.settings.column_inputs_surf = settings.font.render(settings.column_inputs, True,(0,0,255))

				settings.row_inputs_rect = settings.row_inputs_surf.get_rect(center = settings.row_inputs_box.center)
				settings.column_inputs_rect = settings.column_inputs_surf.get_rect(center = settings.column_inputs_box.center)
				...
				settings.row_inputs_rect.center = settings.row_inputs_box.center
				settings.column_inputs_rect.center = settings.column_inputs_box.center

															
					

			case "column" if self.settings.generate_map:
				...
				self.row_in : int = int(len(settings.inputs_entering) // 2) # How many inputs top column will use -> int
				# How many inputs left row will use -> int
				self.col_in : int = int(len(settings.inputs_entering) - (len(settings.inputs_entering) // 2))

				# How many cells should I use for the top column -> int(input possibilities of the top column)
				settings.map_row : int = 2 ** (len(settings.inputs_entering) // 2)
				# How many cells should I use for the left row -> int (input possibilities of the left row)
				settings.map_column : int = 2 ** (len(settings.inputs_entering) - (len(settings.inputs_entering) // 2))

				for input_index, input_type in enumerate(self.settings.input_value.keys()):

					if input_index <= self.col_in - 1:
						settings.column_inputs = settings.column_inputs + input_type
	...
	def display_inputs(self, mouse_pos, width, height):

		# getting the mouse position on the surface
		# When we reduce surface_topleft position values from mouse_pos we will have the values of mouse_pos on the surface
		# 200 is x value of surface's topleft point, 110 is y value of surface's topleft point
		x, y = mouse_pos[0] - 200, mouse_pos[1] - 110
		# Now we have to set x, y values of mouse_pos on the surface according to the starting_cell(rectangle)
		# In normal graph origin(0,0) is bottomleft, but to calculate this we have to set the origin to the topright
		# when we reduce rect_width(80) from x value on the surface we will have mouse_pos.x on the rect
		# We already have y value on the rect
		x = 80 - x

		answer = (width * x + height * y) - (width*height)

		# if answer is smaller than 0 then that means user want to display input types used for the column
		if answer < 0:
			
			self.settings.triangle_column_colour = (255,0,0)
			self.settings.triangle_row_colour = (0,0,0)
			self.settings.display_input = "column_input"


		elif answer > 1: # if not smaller than 0 then that means user want to display input types used for the row
			
			
			self.settings.triangle_row_colour = (255,0,0)
			self.settings.triangle_column_colour = (0,0,0)
			self.settings.display_input = "row_input"
	...
	def generate_possibilities(self) -> None:
		...
		match self.settings.format:
			...
			case "column":
				...
				for binary in range(self.settings.map_row): # binary value is integer whole number

					self.settings.cells_row.append(deepcopy(self.cell_rect)) # Copying the default rect to the list
					self.settings.cells_row[-1].topleft = self.settings.cells_row[-2].bottomleft # shift the rects to the bottom
			
					convert : str = f"{binary:b}" # A f-string method to convert a integer to a boolean value
			
					# If length of the boolean string is smaller than number of input type used on the left-rew then add zeros to the 
					# beginning of the string |v|
					# because even if you have 4 different input type you can get something like "10" it ignores the zeros in the beginning
			
					if len(convert) < self.row_in:
						multi = self.row_in - len(convert) # Calculating how many zeros I should add to the beginning of the string
						convert = f"{'0' * multi}" + convert # Adding the zeros to the beginning of the string

					# Adding the possibility of the left-row cells to the list as a Font (used for display the left-row possibilities)
					self.settings.row_possibilities.append(self.settings.font.render(f"{convert}", True, (0,255,0)))
			
					# Adding the actual result (boolean string) to another list (this list will be used to calculate result cells value)
					self.settings.calculation_values_row.append(f"{convert}")

				self.settings.cells_row.pop(0) # deleting the first rect because we dont need this anymore.
		

				# ***************************** GENERATING THE POSSIBILITIES OF THE TOP COLUMN ***************************************
		
				# A loop for generating the input possibilities for the Top_Column (2^n)
				# I will take the number of input possibilities used for the Top-Column then turning it into a boolean expression
				# iterating through the number of row possibilities to turn every possibility to binary
		
				for binary2 in range(self.settings.map_column): # binary2 value is integer whole number

					self.settings.cells_column.append(deepcopy(self.cell_rect)) # Copying the default rect to the list
					self.settings.cells_column[-1].topleft = self.settings.cells_column[-2].topright # shifts the rect to the right
					...
					if len(convert2) < self.col_in:
						multi2 = self.col_in - len(convert2) # Calculatin how many zeros I should add to the beginning of the string
						convert2 = f"{'0' * multi2}" + convert2 # Adding the zeros to the beginning of the string

					# Adding the possibility of the top_column cells to the list as a Font(used for display the top-column possibilities)
					self.settings.column_possibilities.append(self.settings.font.render(f"{convert2}", True, (0,255,0)))

					# ------------------------- OTHER CELLS SET UP-----------------------------------
					# Copying the Top Rect list for creating the other cells(result rects)
					self.settings.other_cells.append(deepcopy(self.settings.cells_row))
					self.settings.calculated_cells.append([])

					# A loop that shift down last element list of (vertical) result rect lists
					for cells_index, other_cells in enumerate(self.settings.other_cells[-1]):
						# Shifting the result rectangle to the right
						self.settings.other_cells[-1][cells_index].right = self.settings.cells_column[-1].right

						# Conjugenate the col boolean string value to row string value
						# combined string have same length of input_types_dictionary_keys
						self.settings.combine = f"{convert2}{self.settings.calculation_values_row[cells_index]}"
				...
				if len(self.settings.row_possibilities) == 1:
			
					self.settings.row_possibilities[0] = self.settings.font.render("", True, (0,255,0))


				self.settings.cells_column.pop(0)
	...
	def draw(self):
		...
		match self.settings.format:
			...
			case "row":
				...
				#self.display(self.display_input)
				pygame.draw.polygon(self.settings.k_map_bed, self.settings.triangle_column_colour, self.triangle_column_pos)
				#self.settings.screen.blit(self.column_inputs_surf, self.column_inputs_surf.get_rect(midbottom = self.settings.cell_start.midtop))
				pygame.draw.polygon(self.settings.k_map_bed, self.settings.triangle_row_colour, self.triangle_row_pos)
				#self.settings.screen.blit(self.settings.row_inputs_surf, self.row_inputs_surf.get_rect(midright = self.settings.cell_start.midleft))
				if self.settings.display_input == "column_input":

					pygame.draw.rect(self.settings.screen, (255,0,255), self.settings.column_inputs_box)
					self.settings.screen.blit(self.settings.column_inputs_surf, self.settings.column_inputs_rect)

				elif self.settings.display_input == "row_input":

					pygame.draw.rect(self.settings.screen, (255,0,255), self.settings.row_inputs_box)
					self.settings.screen.blit(self.settings.row_inputs_surf, self.settings.row_inputs_rect)
					
				pygame.draw.rect(self.settings.k_map_bed, (255,255,255), self.cell_start, 3)
				pygame.draw.line(self.settings.k_map_bed, (255,255,255), self.cell_start.topleft, self.cell_start.bottomright, 1)

			case "column":

				 # ***************************** Top-Column AND RESULTS *********************************************
				# a loop for drawing the Top-Column, and result rect group
				for draw_index, draw_rect in enumerate(self.settings.cells_column):
	...
	def update(self):
		if self.settings.generate_map: # if the generate_map mode is open then do these functions
			...
			print(f"{'Column_Inputs:':^30}{self.settings.column_inputs}")
			print(f"{'Row_Inputs:':^30}{self.settings.row_inputs}")
			print()
